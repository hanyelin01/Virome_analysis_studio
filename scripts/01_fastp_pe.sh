...
MANIFEST='' PARALLEL='' THREADS='' ADAPTER_PROFILE=$FASTP_DEFAULT_ADAPTER_PROFILE ADAPTER_CATALOG=$ADAPTER_CATALOG ADAPTER_REFERENCE=$ADAPTER_SEQUENCE_REFERENCE RESUME=0
while (($#)); do
  case "$1" in
    --manifest|--parallel-samples|--threads-per-sample|--adapter-profile|--adapter-catalog|--adapter-reference)
      (($#>=2)) || die 2 "Missing value for $1"; case "$1" in --manifest) MANIFEST=$2;; --parallel-samples) PARALLEL=$2;; --threads-per-sample) THREADS=$2;; --adapter-profile) ADAPTER_PROFILE=$2;; --adapter-catalog) ADAPTER_CATALOG=$2;; --adapter-reference) ADAPTER_REFERENCE=$2;; esac; shift 2;;
...
python3 "$SCRIPT_DIR/helpers/adapter_evidence.py" validate --catalog "$ADAPTER_CATALOG" --reference "$ADAPTER_REFERENCE" >/dev/null
positive_int "$ADAPTER_REFERENCE_SCAN_READS" || die 2 'ADAPTER_REFERENCE_SCAN_READS must be a positive integer'
...
run_sample() {
  local sample=$1 r1=$2 r2=$3 out1=$4 out2=$5 outdir report evidence scan log; local -a fastp_cmd
  ...
  scan="$outdir/fastp_report/${sample}.adapter_reference_scan.tsv"
  if [[ -s $out1 || -s $out2 || -s $report ]]; then
    if (( RESUME )) && [[ -s $out1 && -s $out2 && -s $report ]]; then
      [[ -s $evidence ]] || python3 "$SCRIPT_DIR/helpers/adapter_evidence.py" summarize --catalog "$ADAPTER_CATALOG" --reference "$ADAPTER_REFERENCE" --profile "$ADAPTER_PROFILE" --fastp-json "$report" --sample "$sample" --output "$evidence"
      [[ -s $scan ]] || python3 "$SCRIPT_DIR/helpers/scan_adapter_reference.py" --reference "$ADAPTER_REFERENCE" --sample "$sample" --r1 "$r1" --r2 "$r2" --max-reads "$ADAPTER_REFERENCE_SCAN_READS" --output "$scan"
  ...
  python3 "$SCRIPT_DIR/helpers/scan_adapter_reference.py" --reference "$ADAPTER_REFERENCE" --sample "$sample" --r1 "$r1" --r2 "$r2" --max-reads "$ADAPTER_REFERENCE_SCAN_READS" --output "$scan"
  ...
  python3 "$SCRIPT_DIR/helpers/adapter_evidence.py" summarize --catalog "$ADAPTER_CATALOG" --reference "$ADAPTER_REFERENCE" --profile "$ADAPTER_PROFILE" --fastp-json "$report" --sample "$sample" --output "$evidence"
