...
def venn_diagram(discovery: list[dict[str, str]]) -> str:
    ...
    labels = [
        (310, 165, count("geNomad")), (740, 165, count("VirSorter2")), (525, 365, count("DIAMOND-NR-virus")),
        (525, 122, count("geNomad", "VirSorter2")), (430, 272, count("geNomad", "DIAMOND-NR-virus")),
        (620, 272, count("VirSorter2", "DIAMOND-NR-virus")), (525, 212, count(*DISCOVERY_TOOLS)),
    ]
    values = "".join(f"<text x='{x}' y='{y}' class='venn-count'>{value:,}</text>" for x, y, value in labels)
    return f"""<div class='venn-wrap'><svg class='venn' viewBox='0 0 1160 440' role='img' aria-label='横向拉伸的 geNomad、VirSorter2 与 DIAMOND-NR-virus 三集合韦恩图'><title>三工具潜在病毒候选序列韦恩图</title><desc>三个集合使用相同大小的横向椭圆，仅表达集合关系，不编码数量比例。图中所有工具总数与各区域数字均为真实 VC 计数。</desc><defs><filter id='venn-shadow' x='-20%' y='-20%' width='140%' height='140%'><feDropShadow dx='0' dy='7' stdDeviation='7' flood-opacity='.18'/></filter><linearGradient id='venn-bg' x1='0' x2='1'><stop stop-color='#edf7fb'/><stop offset='1' stop-color='#f4f0fb'/></linearGradient></defs><rect x='16' y='16' width='1128' height='408' rx='26' class='venn-bg'/><g filter='url(#venn-shadow)'><ellipse cx='400' cy='160' rx='185' ry='125' class='venn-a'/><ellipse cx='650' cy='160' rx='185' ry='125' class='venn-b'/><ellipse cx='525' cy='295' rx='185' ry='125' class='venn-c'/></g><text x='300' y='52' class='venn-label'>geNomad · {totals['geNomad']:,} VC</text><text x='690' y='52' class='venn-label'>VirSorter2 · {totals['VirSorter2']:,} VC</text><text x='430' y='425' class='venn-label'>DIAMOND-NR-virus · {totals['DIAMOND-NR-virus']:,} VC</text>{values}</svg></div>"""
...
def main() -> None:
    ...
    report = document(
        ...
        f"<section class='panel'><h2>① 三工具潜在病毒序列池：横向韦恩图</h2><p class='sub'>三个集合使用相同大小的横向椭圆，仅表达集合关系；工具总数和各交集区域数字均为真实 VC 计数。</p>{venn_diagram(discovery)}</section>"
