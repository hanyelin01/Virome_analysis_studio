...
def venn_diagram(discovery: list[dict[str, str]]) -> str:
    ...
    def legend(tool: str, tier: str, x: int, y: int, style: str) -> str:
        percentage = totals[tool] / total_vc * 100 if total_vc else 0
        return f"<g transform='translate({x} {y})'><rect width='238' height='54' rx='12' class='venn-key {style}'/><text x='15' y='22' class='venn-key-name'>{esc(tool)} · {esc(tier)}档</text><text x='15' y='42' class='venn-key-value'>{totals[tool]:,} VC（{percentage:.2f}%）</text></g>"
    ...
    return f"""<div class='venn-wrap'><svg class='venn' viewBox='0 0 1160 440' role='img' aria-label='按大中小档次展示 geNomad、VirSorter2 与 DIAMOND-NR-virus 的横向韦恩图'><title>分级大小的三工具病毒候选韦恩图</title><desc>圆的大小只表达检出规模的高、中、低档次，不是定量面积。最大集合 DIAMOND-NR-virus 位于左侧；每个工具的总数及所有交集数量均为真实计数。</desc><defs><filter id='venn-shadow' x='-20%' y='-20%' width='140%' height='140%'><feDropShadow dx='0' dy='7' stdDeviation='7' flood-opacity='.18'/></filter><linearGradient id='venn-bg' x1='0' x2='1'><stop stop-color='#edf7fb'/><stop offset='1' stop-color='#f4f0fb'/></linearGradient></defs><rect x='16' y='16' width='1128' height='408' rx='26' class='venn-bg'/><g filter='url(#venn-shadow)'><circle cx='400' cy='230' r='165' class='venn-c'/><circle cx='590' cy='230' r='118' class='venn-b'/><circle cx='515' cy='130' r='76' class='venn-a'/></g><text x='365' y='260' class='venn-main-label' text-anchor='middle'>DIAMOND-NR-virus</text><text x='365' y='289' class='venn-main-value' text-anchor='middle'>{totals['DIAMOND-NR-virus']:,} VC</text><path d='M 850 107 L 561 151' class='venn-leader'/><path d='M 850 178 L 689 212' class='venn-leader'/><path d='M 850 249 L 550 276' class='venn-leader'/>{legend('geNomad', '小', 850, 80, 'venn-a-key')}{legend('VirSorter2', '中', 850, 151, 'venn-b-key')}{legend('DIAMOND-NR-virus', '大', 850, 222, 'venn-c-key')}<text x='520' y='392' class='venn-exact' text-anchor='middle'>{esc(exact)}</text></svg></div>"""
...
def main() -> None:
    ...
    report = document(
        ...
        f"<section class='panel'><h2>① 三工具潜在病毒序列池：分级横向韦恩图</h2><p class='sub'>圆的大小仅表达检出规模的“大 / 中 / 小”档次，不作为定量面积；图中每种工具的总数和底部交集数均为真实计数。</p>{venn_diagram(discovery)}</section>"
