...
ASSEMBLY_DIR='' CLEAN_DIR='' CLEAN_LAYOUT='' READ_TYPE='' OUTPUT_DIR='' THREADS='' GROUPS_FILE='' MIN_LENGTH="$VIRAL_MIN_CONTIG_LEN" OVERVIEW_RANK="$BATCH_OVERVIEW_RANK" ENABLE_VIRSORTER2=0 RESUME=0
while (($#)); do
  case "$1" in
    --assembly-dir|--cleandata-dir|--clean-layout|--read-type|--output-dir|--threads|--groups-file|--min-contig-length|--overview-rank)
      (($# >= 2)) || die 2 "Missing value for $1"
      case "$1" in
        --assembly-dir) ASSEMBLY_DIR=$2;; --cleandata-dir) CLEAN_DIR=$2;; --clean-layout) CLEAN_LAYOUT=$2;; --read-type) READ_TYPE=$2;; --output-dir) OUTPUT_DIR=$2;; --threads) THREADS=$2;; --groups-file) GROUPS_FILE=$2;; --min-contig-length) MIN_LENGTH=$2;; --overview-rank) OVERVIEW_RANK=$2;;
      esac
      shift 2;;
...
abundance=(bash "$SCRIPT_DIR/07_votu_abundance.sh" --manifest "$MANIFEST" --input "$OUTPUT_DIR/03_checkv/viral_candidates_checkv.fna" --output-dir "$OUTPUT_DIR" --threads "$THREADS"); (( RESUME )) && abundance+=(--resume); run_step votu_abundance "${abundance[@]}"
report=(bash "$SCRIPT_DIR/08_generate_viral_report.sh" --output-dir "$OUTPUT_DIR" --manifest "$MANIFEST" --overview-rank "$OVERVIEW_RANK"); (( RESUME )) && report+=(--resume); run_step report "${report[@]}"
