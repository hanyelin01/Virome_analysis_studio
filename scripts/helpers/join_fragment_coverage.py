...
def number(row: dict[str, str], needle: str) -> str:
    for key, value in row.items():
        if needle in key.lower(): return value
    return "0"
...
def main() -> None:
    ...
    with args.output.open("w", newline="", encoding="utf-8") as handle:
        ...
        for row in annotations:
            ...
            row.update({"relative_abundance": number(value, "relative abundance"), "mean_coverage": number(value, " mean"), "covered_bases": number(value, "covered bases"), "read_count": number(value, " count")})
